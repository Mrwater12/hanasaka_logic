...
export class Game {
    ...
    update() {
        ...
        for (let p of this.players) {
            ...
            if (this.mapData[p.y][p.x] === TILE.SWITCH) {
                ...
                if (swObj && !swObj.isPressed) {
                    ...
                    // ★変更点2: ここですぐ反転せず、フラグを立てるだけにする
                    // this.flipAllArrows(); 
                    requestArrowFlip = true;
                }
        ...
        for (let p of this.players) {
            ...
            if (nextTile === TILE.NONE) {
                ...
                if (p.hasCan) {
                    ...
                    this.mapData[nextY][nextX] = TILE.BLOOM_FLOWER; 
                    p.isFinished = true;
                } else {
                    isGameOver = true; 
                    failMsg = "水がありません！"; 
        ...
        if (!isGameOver) {
            for (let i = 0; i < this.players.length; i++) {
                for (let j = i + 1; j < this.players.length; j++) {
                    ...
                    if (p1.isJumping !== p2.isJumping) continue;
        ...
        // ★変更点3: すべての移動計算が終わったあとに、矢印を反転させる
        if (requestArrowFlip) {
            this.flipAllArrows();
        }
    ...
    calculateFireTiles() {
        ...
        dragons.forEach(d => {
            ...
            for (let i = 1; i <= 3; i++) {
                ...
                if ([TILE.WARP, TILE.SPRING, TILE.DRAGON, TILE.FIRE_BUTTON].includes(tile)) {
                    break;
                }
